...
Array.prototype.last = function () {
  ...
  let game_window =  document.querySelector('#container');
  let height = 600;
  let width = 1000;
  ...
  canvas.width = width; // Make the Canvas full screen
  ...
  game_window.addEventListener("keydown", function (event) {
    if (event.key == " ") {
  ...
  function animate(timestamp) {
    ...
    switch (phase) {
      ...
      case "falling": {
        ...
        const maxHeroY =
          platformHeight + 100 + (height - canvasHeight) / 2;
        if (heroY > maxHeroY) {
