...
let horizontalPadding = (width - mainAreaWidth) / 2;
let verticalPadding = (height - mainAreaHeight) / 2;
...
function resetGame() {
  ...
  trees = [];
  for (let i = 1; i < width / 50; i++) generateTree();

  backgroundTrees = [];
  for (let i = 1; i < width / 30; i++) generateBackgroundTree();
...
function generateTree() {
  ...
  const maximumGap = height; // Maximum distance between two trees
...
window.addEventListener("resize", function () {
  canvas.width = width;
  canvas.height = height;
  horizontalPadding = (width - mainAreaWidth) / 2;
  verticalPadding = (height - mainAreaHeight) / 2;
  draw();
});
...
function drawHeader() {
  ...
  ctx.fillText(`${score} m`, width - 30, 30);
}
