...
<h1 class="title font-weight-normal"><i class="fad fa-exclamation-triangle"></i> Gestion des avertissements</h1>
					...
					<table class="table table-hover mt-3">
						<thead class="bg-light">
							<tr>
								<th>#</th>
								<th>Utilisateur
									<a href="<?= $this->url('back_warning_list');?>?sort=username&dir=<?=$order['username']?>" class="text-dark"><i class="ml-2 sort fas fa-sort"></i>
									</a>
								</th>
								<th>Raison</th>
								<th>Depuis le
									<a href="<?= $this->url('back_warning_list');?>?sort=date_create&dir=<?=$order['date_create']?>" class="text-dark"><i class="ml-2 sort fas fa-sort"></i>
									</a>
								</th>
								<th>Expire le
									<a href="<?= $this->url('back_warning_list');?>?sort=date_expire&dir=<?=$order['date_expire']?>" class="text-dark"><i class="ml-2 sort fas fa-sort"></i>
									</a>
								</th>
								<th>Auteur
									<a href="<?= $this->url('back_warning_list');?>?sort=author&dir=<?=$order['author']?>" class="text-dark"><i class="ml-2 sort fas fa-sort"></i>
									</a>
								</th>
								<th>Action</th>
							</tr>
						...
						<?php foreach ($warning as $item) : ?>
							...
							<tbody>
								<tr>
									...
									<td>
										<a href="<?= $this->url('back_warning_delete', ['id' => $item['id']]); ?>" class="red-text"><i class="fas fa-sm fa-times mr-1"></i>Supprimer</a>
									</td>
								</tr>
							</tbody>
						<?php endforeach; ?>
