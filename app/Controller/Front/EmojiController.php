...
class EmojiController extends MasterFrontController
{
	...
	/**
	 * @var string FileName of official emoji dataFile (http://www.unicode.org/Public/emoji/).
	 */
	public $filePath = '/assets/libs/emojipopper/emoji-test.txt';
	...
	public function ajaxResult()
	{
		if(isset($_SERVER['W_BASE']) && !empty($_SERVER['W_BASE'])){
			$document_root = $_SERVER['DOCUMENT_ROOT'].$_SERVER['W_BASE'];
		}
		else {
			$document_root = $_SERVER['DOCUMENT_ROOT'];
		}
		$this->parseFile(str_replace('//', '/', $document_root.$this->filePath));
		$this->sendResult();
	}
	...
	public function parseFile($filePath)
	{
		...
		while (($buffer = fgets($fileHandle, 4096)) !== false) {
			...
			preg_match('/ subgroup: (.*)/', $buffer, $subgroup, PREG_OFFSET_CAPTURE, 0);

			if (isset($group[1])) {
				$groupName = trim($group[1][0]);
			}
			if (isset($subgroup[1])) {
				$subgroupName = trim($subgroup[1][0]);
			}
